...
| [`lunax.util`](docs/util.md) | 通用工具 — 格式化输出 `dump`、字符串 `trim`/`split`、深度 `clone`、字节格式化 `hsz`、表顺序迭代器 `spairs`、安全数组迭代器 `sipairs`、数组判断 `is_array`、格式化 Lua 风格类型错误 `fmt_type_err` |
...
| [`lunax.archive`](docs/archive.md) | 封装了 Unix zip/unzip，Windows Powershell Compress/Expand-Archive 命令用于 zip 压缩文件的压缩与解压（对于其他格式，建议直接使用跨平台通用 tar 命令 （Windows 10 1803+ 内置）） |
| [`lunax.curl`](docs/curl.md) | 对于 `curl` 命令行工具（ Windows 10 1803+ ) 的封装，支持 HTTP 和文件传输 FTP/SFTP/SCP 以及 接收/发送 邮件 SMTP/IMAP/POP3 |
| [`lunax.timer`](docs/timer.md) | 跨平台 *休眠* 与 *定时任务* 调度模块，依赖 Linux `systemd` 或 macOS `launchctl` 或 Windows `schtasks` |
| [`lunax.tty`](docs/tty.md) | 对于 Unix `stty` 和 Windows `mode` 的封装（目前实现了：获取终端大小；未来会支持更多功能）
