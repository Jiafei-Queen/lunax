...
自动检测 stdout 是否为 TTY，非终端环境下所有输出降级为空字符串，避免污染管道或文件。
...
| 函数 | 说明 |
|------|------|
| `.move_to(row, col)` | 移动光标到指定行列 |
| `.cursor_up(n)` | 光标上移 n 行（默认 1） |
| `.cursor_down(n)` | 光标下移 n 行 |
| `.cursor_right(n)` | 光标右移 n 列 |
| `.cursor_left(n)` | 光标左移 n 列 |
...
print(ansi.bg_red("红底白字效果"))  -- 配合白色前景更佳
